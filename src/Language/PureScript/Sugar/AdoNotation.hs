...
desugarAdo d =
  ...
  pure' ss m = Var ss (Unresolved (Qualified m (Ident C.pure')))

  map' :: SourceSpan -> Maybe ModuleName -> Expr
  map' ss m = Var ss (Unresolved (Qualified m (Ident C.map)))

  apply :: SourceSpan -> Maybe ModuleName -> Expr
  apply ss m = Var ss (Unresolved (Qualified m (Ident C.apply)))
  ...
  go ss (yield, args) (DoNotationBind binder val) = do
    ...
    let abs = Abs (VarBinder ss ident)
                  (Case [Var ss (mkUnresolved ident)]
