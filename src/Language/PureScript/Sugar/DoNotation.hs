...
desugarDo d =
  ...
  bind ss m = Var ss (Unresolved (Qualified m (Ident C.bind)))

  discard :: SourceSpan -> Maybe ModuleName -> Expr
  discard ss m = Var ss (Unresolved (Qualified m (Ident C.discard)))
  ...
  go pos m (DoNotationBind binder val : rest) = do
    ...
    case binder' of
      ...
      _ -> do
        ident <- freshIdent'
        return $ App (App (bind pos m) val) (Abs (VarBinder pos ident) (Case [Var pos (mkUnresolved ident)] [CaseAlternative [binder] [MkUnguarded rest']]))
