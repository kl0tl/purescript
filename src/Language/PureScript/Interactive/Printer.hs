        ...
        showTypeClass (P.Qualified _ name, Just P.TypeClassData{..}) =
            let constraints =
                    ...
                    then Box.text ""
                         ...
                         Box.<> Box.hcat Box.left (intersperse (Box.text ", ") $ map (\(P.Constraint _ (P.Resolved _ (P.Qualified _ pn)) _ lt _) -> textT (P.runProperName pn) Box.<+> Box.hcat Box.left (map (P.typeAtomAsBox maxBound) lt)) typeClassSuperclasses)
                         Box.<> Box.text ") <= "
                className =
